...
static const u8 *GetInteractedBackgroundEventScript(struct MapPosition *position, u8 metatileBehavior, u8 direction)
{
    ...
    if (gMapHeader.mapLayoutId == LAYOUT_OLDALE_TOWN && bgEvent->x == 30 && bgEvent->y == 5 && FlagGet(FLAG_TANGELO_LAPRAS) == TRUE)
        return NULL;
...
static const u8 *GetInteractedMetatileScript(struct MapPosition *position, u8 metatileBehavior, u8 direction)
{
    ...
    if (MetatileBehavior_IsRefrigerator(metatileBehavior) == TRUE)
        return EventScript_Refrigerator;
    ...
    if (MetatileBehavior_IsFRLGTV(metatileBehavior) == TRUE)
        return EventScript_FRLGTV;
    if (MetatileBehavior_IsStove(metatileBehavior) == TRUE)
        return EventScript_Stove;
    if (MetatileBehavior_IsDishCupboard(metatileBehavior) == TRUE)
        return EventScript_DishCupboard;
    if (MetatileBehavior_IsMachine(metatileBehavior) == TRUE)
        return EventScript_Machine;
    if (MetatileBehavior_IsPhone(metatileBehavior) == TRUE)
        return EventScript_Phone;
    if (MetatileBehavior_IsComputer(metatileBehavior) == TRUE)
        return EventScript_Computer;
    if (MetatileBehavior_IsMachine2(metatileBehavior) == TRUE)
        return EventScript_Machine2;
    if (MetatileBehavior_IsWindow(metatileBehavior) == TRUE)
        return EventScript_Window;
    if (MetatileBehavior_IsPainting1(metatileBehavior) == TRUE)
        return EventScript_Painting1;
    if (MetatileBehavior_IsPainting2(metatileBehavior) == TRUE)
        return EventScript_Painting2;

...
//.rodata
static const u16 sIntroLogo_Pal[] = INCBIN_U16("graphics/intro/intro_logo.gbapal");
static const u32 sIntroLogo_Gfx[] = INCBIN_U32("graphics/intro/intro_logo.4bpp.lz");
static const u32 sIntroLogo_Tilemap[] = INCBIN_U32("graphics/intro/intro_logo.bin.lz");
...
const struct SpriteTemplate gIntroRayquazaHyperbeamSprite =
{
    .tileTag = 2003,
    .paletteTag = 2003,
...
const struct CompressedSpriteSheet gIntroRayquazaGlowSpriteSheet[] =
{
    {gIntro3MiscTiles, 0xA00, 2003},
    {NULL},
};
const struct SpritePalette gIntroRayquazaGlowPalette[] =
{
...
{
    //start moving rock
    if (gIntroFrameCounter == (76-0x60))
		CreateTask(Task_IntroWaterDrops_3, 0);
	
    if (gIntroFrameCounter == (128-0x60))
	{
        CreateTask(Task_IntroWaterDrops_1, 0);
		m4aSongNumStart(MUS_GFINTRO);
	}
	
    if (gIntroFrameCounter == (256-0x60)) 
	{
		//m4aSongNumStart(MUS_ME_RG_PHOTO);
        CreateTask(Task_IntroWaterDrops_2, 0);
	}
    if (gIntroFrameCounter > 350-0x60)
    {
...
static void sub_816DAE8(struct Sprite *sprite)
{
    ...
    case 1:
        ...
        sprite->data[0]++;
    case 2:
        if (sprite->pos1.x + sprite->pos2.x <= -0x60)
        {
            DestroySprite(sprite);
        }
        else
...
void Task_EndIntroMovie(u8 taskId)
{
    DestroyTask(taskId);
    SetMainCallback2(MainCB2_EndIntro);
}
...
static void sub_816FB38(struct Sprite *sprite)
{
    ...
    case 0:
        ...
        break;
    case 1:
        if (gIntroFrameCounter == (0x90-0x60))
        {
            sprite->data[0]++;
            sprite->data[1] = 9;
            sprite->data[3] = 2;
    ...
    case 3:
        ...
        break;
    case 4:
        if (gIntroFrameCounter == (0x110-0x60))
        {
...
static void sub_816FD44(struct Sprite *sprite)
{
    ...
    {
    case 0:
        if (gIntroFrameCounter == (0x80-0x60))
        {
            sprite->invisible = FALSE;
            sprite->data[0]++;
        }
...
static u8 CreatePart1Animations(s16 a0, s16 a1, s16 a2)
{
    ...
    for (i = 0; i < 9; i++)
    {
        spriteId = CreateSprite(&gUnknown_085E4F5C, gUnknown_085E4E94[i][1] + a0, a1 + 3, 0);
        gSprites[spriteId].data[0] = 0;
    ...
    spriteId = CreateSprite(&gUnknown_085E4F8C, 120, a1 - 1, 0);
    gSprites[spriteId].data[0] = 0;
    gSprites[spriteId].invisible = TRUE;
    gSprites[spriteId].oam.matrixNum = i + 12;

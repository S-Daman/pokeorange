...
void ItemUseInBattle_EnigmaBerry(u8 taskId)
{
	...
	//FieldInitRegionMap(sub_80AF6D4);
	
void ItemUseTown_Map(u8 taskId)
{
    if (sub_81221AC() == TRUE)
    {
        DisplayDadsAdviceCannotUseItemMessage(taskId, gTasks[taskId].data[3]);
    ...
    {
        gBagMenu->mainCallback2 = chutiya;
        unknown_ItemMenu_Confirm(taskId);
    }
    else
    {
        gFieldCallback = sub_80AF6D4;
        FadeScreen(1, 0);
        gTasks[taskId].func = chutiya2;
    }
}

void chutiya(void)
{
    Bosda(bag_menu_mail_related);
}

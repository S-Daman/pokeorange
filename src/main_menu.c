 ...
 * Task_NewGameBirchSpeech_StartBirchOddishPlatformFade
 * Task_NewGameBirchSpeech_StartBirchOddishPlatformFade
 ...
 * Task_NewGameBirchSpeech_ReshowBirchOddish
 ...
 *  - Advances to Task_NewGameBirchSpeechSub_WaitForOddish
 * Task_NewGameBirchSpeechSub_WaitForOddish
...
static void Task_NewGameBirchSpeechSub_WaitForOddish(u8);
static void Task_NewGameBirchSpeech_StartBirchOddishPlatformFade(u8);
...
static void Task_NewGameBirchSpeech_ReshowBirchOddish(u8);
...
static const u16 sBirchSpeechPlatformBlackPal[] = INCBIN_U16("graphics/birch_speech/bg2.gbapal");
...
static void Task_NewGameBirchSpeech_Init(u8 taskId)
{
    ...
    gTasks[taskId].tTimer = 0x48;
    PlayBGM(MUS_ORANGE_ROUTETWO);
    ShowBg(0);
    ShowBg(1);
}
...
{
    u8 spriteId = gTasks[sBirchSpeechMainTaskId].tOddishSpriteId;

    gSprites[spriteId].pos1.x = 102;
    gSprites[spriteId].pos1.y = 73;
    gSprites[spriteId].invisible = FALSE;
    gSprites[spriteId].data[0] = 0;

    CreatePokeballSpriteToReleaseMon(spriteId, gSprites[spriteId].oam.paletteNum, 112, 58, 0, 0, 32, 0x0000FFFF, SPECIES_ODDISH);
    gTasks[taskId].func = Task_NewGameBirchSpeechSub_WaitForOddish;
    gTasks[sBirchSpeechMainTaskId].tTimer = 0;
}

static void Task_NewGameBirchSpeechSub_WaitForOddish(u8 taskId)
{
    s16 *data = gTasks[taskId].data;
    struct Sprite *sprite = &gSprites[gTasks[sBirchSpeechMainTaskId].tOddishSpriteId];
...
static void Task_NewGameBirchSpeech_AndYouAre(u8 taskId)
{
    if (!RunTextPrintersAndIsPrinter0Active())
    {
        ...
        AddTextPrinterForMessage(1);
        gTasks[taskId].func = Task_NewGameBirchSpeech_StartBirchOddishPlatformFade;
    }
}

static void Task_NewGameBirchSpeech_StartBirchOddishPlatformFade(u8 taskId)
{
    if (!RunTextPrintersAndIsPrinter0Active())
    {
        gSprites[gTasks[taskId].tBirchSpriteId].oam.objMode = 1;
        gSprites[gTasks[taskId].tOddishSpriteId].oam.objMode = 1;
...
static void Task_NewGameBirchSpeech_StartPlayerFadeIn(u8 taskId)
{
    if (gTasks[taskId].tIsDoneFadingSprites)
    {
        ...
        gSprites[gTasks[taskId].tOddishSpriteId].invisible = TRUE;
        if (gTasks[taskId].tTimer)
        {
            gTasks[taskId].tTimer--;
        }
...
static void Task_NewGameBirchSpeech_StartNamingScreen(u8 taskId)
{
    ...
    {
        FreeAllWindowBuffers();
        FreeAndDestroyMonPicSprite(gTasks[taskId].tOddishSpriteId);
...
static void Task_NewGameBirchSpeech_SlidePlatformAway2(u8 taskId)
{
    ...
    {
        gTasks[taskId].func = Task_NewGameBirchSpeech_ReshowBirchOddish;
    }
}

static void Task_NewGameBirchSpeech_ReshowBirchOddish(u8 taskId)
{
    u8 spriteId;
    ...
    if (gTasks[taskId].tIsDoneFadingSprites)
    {
        ...
        gSprites[spriteId].invisible = FALSE;
        gSprites[spriteId].oam.objMode = 1;
        spriteId = gTasks[taskId].tOddishSpriteId;
...
static void Task_NewGameBirchSpeech_WaitForSpriteFadeInAndTextPrinter(u8 taskId)
{
    ...
    {
        gSprites[gTasks[taskId].tBirchSpriteId].oam.objMode = 0;
        gSprites[gTasks[taskId].tOddishSpriteId].oam.objMode = 0;
        if (!RunTextPrintersAndIsPrinter0Active())
        {
            gSprites[gTasks[taskId].tBirchSpriteId].oam.objMode = 1;
            gSprites[gTasks[taskId].tOddishSpriteId].oam.objMode = 1;
...
static void Task_NewGameBirchSpeech_FadePlayerToWhite(u8 taskId)
{
    ...
    if (!gPaletteFade.active)
    {
        ...
        SetGpuReg(REG_OFFSET_DISPCNT, DISPCNT_OBJ_ON | DISPCNT_OBJ_1D_MAP);
        BeginNormalPaletteFade(0xFFFF0000, 6, 0, 16, RGB_WHITEALPHA);
...
static u8 NewGameBirchSpeech_CreateOddishSprite(u8 a, u8 b)
{
    return CreatePicSprite2(SPECIES_ODDISH, 8, 0, 1, a, b, 14, -1);
}
...
static void AddBirchSpeechObjects(u8 taskId)
{
    ...
    OddishSpriteId = NewGameBirchSpeech_CreateOddishSprite(100, 0x4B);
    gSprites[OddishSpriteId].callback = SpriteCB_Null;
    gSprites[OddishSpriteId].oam.priority = 0;
    gSprites[OddishSpriteId].invisible = TRUE;
    gTasks[taskId].tOddishSpriteId = OddishSpriteId;
...
static const u32 badgeFlagArray[] = {FLAG_BADGE05_GET, FLAG_BADGE01_GET, FLAG_BADGE03_GET, FLAG_BADGE02_GET, FLAG_BADGE07_GET, FLAG_BADGE06_GET, FLAG_BADGE04_GET, FLAG_BADGE08_GET};
...
static void MainMenu_FormatSavegameBadges(void)
{
    ...
    for (i = 0; i < 8; i++)
    {
        if (FlagGet(badgeFlagArray[i]) && i % 2 == 0)
            badgeCount++;
    }

...
u8 sub_80BD930(void)
{
    u16 r2, speciesNum, nationalDexNum;
    ...
    else if ((gMain.newKeys & SELECT_BUTTON) || (sPokedexView->selectedScreen == CANCEL_SCREEN && gMain.newKeys & A_BUTTON))
    {
        r2 = r4;

        speciesNum = NationalPokedexNumToSpecies(sPokedexView->pokedexList[r2].dexNum);
        nationalDexNum = SpeciesToNationalPokedexNum(gBaseStats[speciesNum].form);
        if(GetSetPokedexFlag(nationalDexNum, FLAG_GET_SEEN))
        {
            sPokedexView->pokedexList[r2].dexNum = nationalDexNum;
            sPokedexView->pokedexList[r2].owned = GetSetPokedexFlag(nationalDexNum, FLAG_GET_CAUGHT);
            return TRUE;
        }
        else
        {
...
void sub_80C020C(u32 num, u32 value, u32 c, u32 d)
{
    ...
    if (value == 0)
        ...
        value = num;
    value = CorrectNumberForForm(value);  

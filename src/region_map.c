...
static const u16 Bosda_sRegionMapBkgnd_Pal[] = INCBIN_U16("graphics/pokenav/bosda_region_map_map.gbapal");
static const u32 Bosda_sRegionMapBkgnd_GfxLZ[] = INCBIN_U32("graphics/pokenav/bosda_region_map.8bpp.lz");
static const u32 Bosda_sRegionMapBkgnd_TilemapLZ[] = INCBIN_U32("graphics/pokenav/bosda_region_map_map.bin.lz");
static const u32 Bosda_sRegionMapCursorSmallGfxLZ[] = INCBIN_U32("graphics/pokenav/bosda_cursor_small.4bpp.lz");
static const u32 Bosda_sRegionMapCursorLargeGfxLZ[] = INCBIN_U32("graphics/pokenav/bosda_cursor_large.4bpp.lz");
...
void Bosda_InitRegionMap(struct RegionMap *regionMap, bool8 zoomed)
{
    sub_8122CF8(regionMap, NULL, zoomed);
    while (Bosda_sub_8122DB0());
}
...
bool8 sub_8122DB0(void)
{
    ...
    case 2:
        if (!free_temp_tile_data_buffers_if_possible())
        {
            LoadPalette(sRegionMapBkgnd_Pal, 0x0, 0x6A);
        }
        break;
    case 3:
...
bool8 Bosda_sub_8122DB0(void)
{
    ...
    case 0:
        if (gRegionMap->bgManaged)
        {
            decompress_and_copy_tile_data_to_vram(gRegionMap->bgNum, Bosda_sRegionMapBkgnd_GfxLZ, 0, 0, 0);
        }
        else
        {
            LZ77UnCompVram(Bosda_sRegionMapBkgnd_GfxLZ, (u16 *)BG_CHAR_ADDR(2));
        }
        break;
    case 1:
        if (gRegionMap->bgManaged)
        {
            if (!free_temp_tile_data_buffers_if_possible())
            {
                decompress_and_copy_tile_data_to_vram(gRegionMap->bgNum, Bosda_sRegionMapBkgnd_TilemapLZ, 0, 0, 1);
            }
        }
        else
        {
            LZ77UnCompVram(Bosda_sRegionMapBkgnd_TilemapLZ, (u16 *)BG_SCREEN_ADDR(28));
        }
        break;
    case 2:
        if (!free_temp_tile_data_buffers_if_possible())
        {
            LoadPalette(Bosda_sRegionMapBkgnd_Pal, 0x0, 0x90);
        }
        break;
    case 3:
        LZ77UnCompWram(Bosda_sRegionMapCursorSmallGfxLZ, gRegionMap->cursorSmallImage);
        break;
    case 4:
        LZ77UnCompWram(Bosda_sRegionMapCursorLargeGfxLZ, gRegionMap->cursorLargeImage);
        break;
    case 5:
        RegionMap_InitializeStateBasedOnPlayerLocation();
        gRegionMap->playerIconSpritePosX = gRegionMap->cursorPosX;
